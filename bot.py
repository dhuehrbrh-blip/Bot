...
async def handle_db_upload(message: types.Message):
    ...
    try:
        ...
        from phash_watcher import cursor as main_cursor, conn as main_conn

        src_cursor.execute("SELECT phash FROM photos")
        imported = 0
        skipped = 0

        for (phash,) in src_cursor.fetchall():
            main_cursor.execute(
                "SELECT 1 FROM photos WHERE phash = ?",
                (phash,)
            )
            if main_cursor.fetchone():
                ...
                main_cursor.execute(
                    "INSERT INTO photos (phash, created_at) VALUES (?, datetime('now'))",
                    (phash,)
                )
                imported += 1
